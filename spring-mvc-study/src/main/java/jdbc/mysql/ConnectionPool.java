/***********************************************************************  
 *  
 *   @package：jdbc.mysql,@class-name：ConnsecionsPool.java  
 *   
...
public class ConnectionPool {
	 ...
	 * getConnection(). With some jdbc-drivers, the extra checks can have a
	 * large performance penalty. Default value is 'true'.
	 **/
	 ...
	 * connection that has been idle for a longer time will be removed by the
	 * pool cleaner the next time it check the pool. The default value is 30
	 * seconds.
	 *
	 * @param maxIdleTime
	 ...
	 * connection that has been used for a longer time will be forced to close
	 * itself, even if it is still in use. Normally, this time should only be
	 * reached in case an program "forgets" to close a connection. The maximum
	 * time is switched of by default.
	 *
	 * @param maxUseTime
	 *            The maximum time a connection can be used in milliseconds, or
	 *            a negative value if there is no maximum.
	 **/
